...
PROMPT_TEMPLATE_PRESETS = [
    # Basic 모드 기본 프롬프트
    {
        "mode": "basic",
        "name": "기본 후기 생성",
        "description": "Basic 모드 기본 프롬프트 - 간단한 정보로 자연스러운 후기 생성",
        "is_default": True,
        "content": """당신은 실제로 시술을 받았거나 받을 환자로서 자연스러운 시술후기, 경험, 상담후기, 질문, 고민, 의견, 잡담들을 작성합니다.
...
자연스러운 후기를 작성해주세요:""",
    ...
    # Basic Plus 모드 기본 프롬프트
    {
        "mode": "basic_plus",
        "name": "시리즈 생성",
        "description": "Basic Plus 모드 기본 프롬프트 - 시간 순서대로 일련의 글 생성",
        "is_default": True,
        "content": """당신은 실제로 미용 시술을 받는 사람의 관점에서 시간 순서대로 일련의 글을 작성합니다.
...
지금부터 시리즈를 작성해주세요:""",
    ...
    # Pro 모드 - 헤더
    {
        "mode": "pro_header",
        "name": "기본 헤더",
        "description": "Pro 모드 기본 헤더 - 역할 정의 및 기본 지시사항",
        "is_default": True,
        "content": """당신은 실제로 시술을 받았거나 받을 환자로서 자연스러운 시술후기, 경험, 상담후기, 질문글들을 작성합니다.
광고가 아닌 진짜 의견과 사실, 경험담처럼 작성해주세요.""",
    },
    ...
    # Pro 모드 - 가이드라인
    {
        "mode": "pro_guidelines",
        "name": "기본 가이드라인",
        "description": "Pro 모드 기본 가이드라인 - 작성 시 주의사항",
        "is_default": True,
        "content": """1. 위 정보를 바탕으로 실제 경험한 것처럼 자연스럽게 작성
...
def create_prompt_templates():
    ...
    for preset in PROMPT_TEMPLATE_PRESETS:
        ...
        status = "생성됨" if created else "업데이트됨"
        print(f"  PromptTemplate: [{template.get_mode_display()}] {template.name} - {status}")

    print(f"\n총 {len(PROMPT_TEMPLATE_PRESETS)}개 프롬프트 템플릿 처리 완료")
...
def create_all():
    ...
    print("\n" + "=" * 60)
    print("완료! 총 144개 항목 + 프롬프트 템플릿 설정 완료")
    print("  - 페르소나: 26개 항목 x 6개 프리셋")
    print("  - 카페 셋팅: 21개 항목 x 6개 프리셋")
    print("  - 컨텐츠 유형: 공통 4개 + 타입별 세부 항목 x 6개 프리셋")
    print("  - 프롬프트 템플릿: 4개 (Basic, Basic Plus, Pro Header, Pro Guidelines)")
    print("=" * 60)
